...
def run_and_save(command, log):
    tsk = subprocess.Popen(
        command, stdout=log, stderr=subprocess.STDOUT, shell=True  # subprocess.PIPE,
    )
    tsk.wait()
...
def match_pdb_to_amberff(pdb_file, ff_libs=amber_ff_libs):
    ...
    proton_res = {"CYS": ["CYS", "CYM", "CYX"], "HIS": ["HID", "HIE", "HIP"]}
    ...
    for res in mda_traj.residues:
        ...
        local_mis_atoms = [
            atom for name in local_mis for atom in res.atoms if name == atom.name
        ]
...
def update_pdb_obabel(pdb_file, format="pdb"):
    ...
    subprocess.check_output(
        f"obabel -ipdb {pdb_file} -o{format} >  {pdb_ob}", shell=True
    )
    return pdb_ob
...
def get_lig_charge(pdb_file):
    ...
    if (n_electron % 2 == 0) & (lig_charge % 2 == 0):
        ...
        warnings.warn(
            f"Using 0 for ligand charge. However, number of electron "
        ...
        raise Exception(
            f"Number of electron {n_electron} and charge {lig_charge} "
            f"are mismatch for ligand {os.path.abspath(pdb_file)}"
        )
...
def clean_pdb(pdb_file):
    ...
    with open(pdb_file, "r") as pdb:
        pdb_atoms = [
            line for line in pdb if line.startswith("ATOM") or line.startswith("HETATM")
        ]
...
def add_hydrogen(pdb_file):
    ...
    subprocess.check_output(f"obabel -ipdb {pdb_noH} -h -opdb >  {pdb_h}", shell=True)
    clean_pdb(pdb_h)
    return pdb_h
